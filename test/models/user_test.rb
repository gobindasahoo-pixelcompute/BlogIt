...
class UserTest < ActiveSupport::TestCase
  ...
  def test_user_should_not_be_valid_and_saved_if_email_not_unique
    ...
    test_user = build(:user, email: @user.email, organization: @organization)
  ...
  def test_validation_should_reject_invalid_addresses
    invalid_emails = %w[user@example,com user_at_example.org user.name@example.
                        @sam.com sam@sam+exam.com fishy+#.com]
  ...
  def test_users_should_have_unique_auth_token
    @user.save!
    second_user = create(:user, organization: @organization)

    refute_equal @user.authentication_token, second_user.authentication_token
  end

  def test_user_should_not_be_valid_without_organization_id
